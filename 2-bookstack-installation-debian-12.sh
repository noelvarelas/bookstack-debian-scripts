...
function run_pre_install_checks() {
  ...
  # Check if MariaDB appears to be installed and exit if so
  if [ -d "/var/lib/mysql" ]
  then
    error_out "This script is intended for a fresh server install, existing MariaDB data found, aborting install"
  fi
}

# Asks if a restore is needed using previously created backup files
# Uses a .sql and .tar.gz as demonstrated in BookStack documentation
function run_pre_install_restoration_check() {
echo; [[ "$(read -e -p 'Do you want to restore a backup? [y/N] '; echo "$REPLY")" == [Yy]* ]] && ENABLE_RESTORE="true"
if [ "$ENABLE_RESTORE" = "true" ]; then
  echo "Enter the path of your .sql backup file:"
  read -r PROVIDED_DATABASE
  if ! [[ -f "$PROVIDED_DATABASE" && "$PROVIDED_DATABASE" == *.sql ]]; then
    echo "Invalid file. Exiting..."
    exit 1
  fi

  echo "Enter the path of your .tar.gz backup file:"
  read -r PROVIDED_FILES
  if ! [[ -f "$PROVIDED_FILES" && "$PROVIDED_FILES" == *.tar.gz ]]; then
    echo "Invalid file. Exiting..."
    exit 1
  fi

  echo "Backup files accepted. Your data will be restored."
  DATABASE_RESTORE_FILE=$(realpath "$PROVIDED_DATABASE")
  FILES_RESTORE_FILE=$(realpath "$PROVIDED_FILES")
fi
}
...
function run_prompt_for_domain_if_required() {
  ...
  then
    info_msg ""
    info_msg "Enter the full http or https address you want to host BookStack on and press [ENTER]."
    info_msg "Examples: 'https://docs.my-site.com' or 'http://${CURRENT_IP}'"
    read -r DOMAIN
  fi
...
function run_update_bookstack_env() {
  cd "$BOOKSTACK_DIR" || exit
  cp .env.example .env
  sed -i.bak "s@APP_URL=.*\$@APP_URL=$DOMAIN@" .env
...
# Update backup's environment variables with new DB password and URL
function run_restoration() {
  cd "$BOOKSTACK_DIR" || exit
  mysql -u root -p bookstack < "$DATABASE_RESTORE_FILE"
  tar -xzf "$FILES_RESTORE_FILE" -C "$BOOKSTACK_DIR"
  sed -i.bak "s/DB_PASSWORD=.*\$/DB_PASSWORD=$DB_PASS/" .env

  # Migrate to new domain if different from backup
  DOMAIN_OLD=$(grep "^APP_URL=" "$BOOKSTACK_DIR/.env" | cut -d '=' -f 2)
  if [[ "$DOMAIN" != "$DOMAIN_OLD" ]]; then
    sed -i.bak "s@APP_URL=.*\$@APP_URL=$DOMAIN@" .env
    php artisan bookstack:update-url --force "$DOMAIN_OLD" "$DOMAIN"
  fi
}
...
run_pre_install_restoration_check
...
info_msg "[2/9] Preparing MariaDB database..."
...
info_msg "- Access URL: http://$CURRENT_IP/ or $DOMAIN/"
